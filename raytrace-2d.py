...
R1 = 120.0 #Radius der ersten Kugel
...
brennweite = Linsenschleifergleichung()*0.334 # praktische Brennweite mit komischem Korrekturfaktor
print(Linsenschleifergleichung())


q1 = 2.0*brennweite+R1-d1
...
def SchnittpunktKreis(a, b, q, r, index): # Bestimmt den Schnittpunkt zwischen einer Gerade a+k*b und einem Kreis Kugel des Radiuses r um den Punkt z 
     C1 = sp(b, b)
...
pos2 = np.array([0.0, 40.0])
pos3 = np.array([0.0, 30.0])
pos4 = np.array([0.0, 20.0])
pos5 = np.array([0.0,  10.0])

xAxZ = np.linspace(0, 4.5*brennweite, num) #
yAxZ = 0.0*xAxZ # optische Achse

v2 = np.array([1.0, 0.0]) # Es wird in diesem Beispiel nur paralleles Licht betrachtet
...
xa = np.array([0.0, 2.0*brennweite, 4.0*brennweite])
plt.xticks(xa, ['0', '2f', '4f'])
plt.legend()
plt.show()
